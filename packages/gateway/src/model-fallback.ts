...
export class ModelFallbackController {
  ...
  async onPromptError(consecutiveFailures: number): Promise<boolean> {
    ...
    return this._activateFallback(`${consecutiveFailures} consecutive prompt failures`);
  }
  ...
  private async _activateFallback(reason: string): Promise<boolean> {
  ...
  private async _maybeRecoverPrimary(): Promise<void> {
    ...
    try {
      ...
      console.log("[gateway:fallback] recovered to primary", {
        primary: `${this.primaryModel.provider}/${this.primaryModel.id}`,
      });
      this.notify(msg);
      ...
      void emitGatewayOtel({
        level: "info",
        component: "daemon.fallback",
        action: "fallback.recovered",
        success: true,
        metadata: {
          primaryProvider: this.primaryModel.provider,
          primaryModel: this.primaryModel.id,
          fallbackDurationMs: Date.now() - (this._activeSince || Date.now()),
          immediateTelegram: true,
        },
      });
    } catch (error) {
      ...
      void emitGatewayOtel({
        level: "debug",
        component: "daemon.fallback",
        action: "fallback.recovery_probe.failed",
        success: false,
        error: String(error),
      });
    }
