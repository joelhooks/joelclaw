...
const TYPESENSE_URL =
  process.env.DOCS_TYPESENSE_URL || process.env.TYPESENSE_URL || "http://localhost:8108";
...
async function fetchSnippetsForParentSection(
  ...
  const params = new URLSearchParams({
    ...
    include_fields:
      "id,doc_id,title,chunk_type,chunk_index,heading_path,context_prefix,parent_chunk_id,prev_chunk_id,next_chunk_id,primary_concept_id,concept_ids,concept_source,taxonomy_version,source_entity_id,evidence_tier,parent_evidence_id,content,retrieval_text",
    exclude_fields: "embedding",
  });
...
function buildDocsSearchParams(
  query: string,
  limit: number,
    ...
    Effect.gen(function* () {
      try {
            ...
            [
              {
                ...
                params: {
                  "chunk-id": {
                    required: true,
                    value: hits[0]?.id,
                    description: "Chunk ID from hit list",
                  },
                },
              },
              {
        ...
        if (isTypesenseApiKeyError(error)) {
          yield* Console.log(
            respondError("docs search", error.message, error.code, error.fix, [
        ...
        yield* Console.log(
          respondError(
            ...
            [
              ...
              {
                command: 'joelclaw otel search "docs." --hours 1',
                description: "Inspect docs OTEL events",
              },
            ],
          ),
        );
  ...
  ({ chunkId, mode, before, after, neighbors }) =>
    Effect.gen(function* () {
      try {
              ...
              [
                {
                  ...
                  description: "Find chunk IDs first",
        ...
        if (isTypesenseApiKeyError(error)) {
          yield* Console.log(
            respondError("docs context", error.message, error.code, error.fix, [
      ...
      yield* Console.log(
        respond(
          ...
          [
            ...
            {
              command: 'joelclaw otel search "docs." --hours 1',
              description: "Check docs ingest telemetry",
            },
    ...
    Effect.gen(function* () {
      try {
        ...
        const params = new URLSearchParams({
          ...
          include_fields:
            "id,title,filename,nas_path,nas_paths,storage_category,document_type,file_type,tags,primary_concept_id,concept_ids,taxonomy_version,added_at,size_bytes",
          exclude_fields: "embedding",
        });
        if (categoryValue) {
        ...
        const result = yield* Effect.promise(() =>
          typesenseSearch(apiKey, DOCS_COLLECTION, params),
        );
        ...
        yield* Console.log(
          respond(
            ...
            [
              ...
              {
                command: "joelclaw docs search <query>",
                description: "Search chunk evidence",
                params: { query: { required: true } },
              },
            ],
          ),
        ...
        if (isTypesenseApiKeyError(error)) {
          yield* Console.log(
            respondError("docs list", error.message, error.code, error.fix, [
              { command: "joelclaw status", description: "Check system status" },
            ]),
          );
          return;
    ...
    Effect.gen(function* () {
      try {
        ...
        const statsParams = new URLSearchParams({
          ...
          include_fields:
            "id,chunk_type,chunk_index,context_prefix,parent_chunk_id,evidence_tier,content",
          exclude_fields: "embedding,retrieval_text",
        });
        ...
        if (isTypesenseApiKeyError(error)) {
          yield* Console.log(
            respondError("docs show", error.message, error.code, error.fix, [
      ...
      try {
            ...
            [
              ...
              },
              {
                ...
                params: {
                  "chunk-id": {
                    required: true,
                    description: "Chunk id from `joelclaw docs search`",
                  },
                },
              },
              {
        ...
        yield* Console.log(
          respondError(
            ...
            [
              {
                command: "joelclaw docs status",
                description: "Check docs collection and artifact status",
              },
            ],
          ),
        );
...
const statusCmd = Command.make("status", {}, () =>
  Effect.gen(function* () {
    try {
      ...
      const safeTsSearch = async (
        collection: string,
        params: URLSearchParams,
      ): Promise<TypesenseSearchResponse> => {
        try {
          return await typesenseSearch(apiKey, collection, params);
        } catch {
          return { found: 0, hits: [], facet_counts: [] };
        }
      };
      const [docsResult, chunksV1Result, chunksV2Result, artifactsAvailable] =
      ...
      yield* Console.log(
        respond(
          ...
          [
            {
              command: "joelclaw docs list --limit 10",
              description: "Inspect latest indexed docs",
            },
            {
              command: "joelclaw docs search <query>",
      ...
      if (isTypesenseApiKeyError(error)) {
        yield* Console.log(
          respondError("docs status", error.message, error.code, error.fix, [
  ...
  ({ manifest, sample }) =>
    Effect.gen(function* () {
      try {
        ...
        const pathExactMissing = manifestBookTargetsExisting.filter(
          (path) => !indexedPathExactSet.has(normalizePathKey(path)),
        );
        ...
        const contentEquivalentMissing = manifestBookTargetsExisting.filter(
          (path) => !indexedContentEquivalentSet.has(contentEquivalentKey(path)),
        );
        ...
        yield* Console.log(
          respond(
            ...
            [
              {
                command: "joelclaw docs status",
                description: "Verify docs/chunks collection health",
              },
              {
                command: "joelclaw docs list --limit 20",
                description: "Inspect current indexed docs",
              },
              {
                command: 'joelclaw otel search "docs.path.aliases.updated" --hours 24',
                description: "Confirm alias writes from ingest",
              },
            ],
          ),
        );
        ...
        if (isTypesenseApiKeyError(error)) {
          yield* Console.log(
            respondError("docs reconcile", error.message, error.code, error.fix, [
              { command: "joelclaw status", description: "Check system status" },
            ]),
          );
          return;
        ...
        yield* Console.log(
          respondError(
            ...
            [
              ...
              {
                command: "joelclaw runs --count 10 --hours 24",
                description: "Inspect recent docs pipeline runs",
              },
            ],
          ),
        );
    ...
    respond(
      "docs",
      {
        ...
        subcommands: {
          ...
          search:
            "joelclaw docs search <query> [--limit N] [--category <category>] [--concept <concept-id>] [--chunk-type <section|snippet>] [--doc <doc-id>]",
          context:
            "joelclaw docs context <chunk-id> [--mode snippet-window|parent-section|section-neighborhood]",
      ...
      [
        ...
        {
          command: "joelclaw docs search <query>",
          description: "Run snippet-first retrieval",
          params: { query: { required: true } },
        },
        { command: "joelclaw docs list --limit 10", description: "Inspect indexed docs" },
        {
          command: "joelclaw docs add <path>",
          description: "Queue ingest for a file path",
