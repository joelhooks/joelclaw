    ...
    Effect.gen(function* () {
      const events = {
        ...
        "pipeline/transcript.process": {
          ...
          example: `joelclaw send pipeline/transcript.process -d '{"source":"granola","text":"...","title":"Meeting","slug":"meeting"}'`,
        },
        "content/summarize": {
          description: "Enrich a vault note with pi + web research",
          data: { vaultPath: "string (required)", prompt: "string (optional)" },
          example: `joelclaw send content/summarize -d '{"vaultPath":"/Users/joel/Vault/Resources/videos/some-note.md"}'`,
        },
        "system/log": {
      ...
      yield* Console.log(respond("schema", events, [
        { command: `joelclaw send pipeline/video.download --url "https://youtube.com/watch?v=XXXX"`, description: "Download a video" },
