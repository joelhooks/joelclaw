...
import {
  ...
  inspectWorkloadStageDependencies,
  isNestedWorkflowRigSandboxExecution,
  parseWorkloadTaskIdentity,
...
const stagesFromOption = Options.text("stages-from").pipe(
  Options.withDescription(
    "Load explicit stage definitions from a JSON file instead of auto-generating stages",
  ),
  Options.optional,
);
...
const runSkipDepCheckOption = Options.boolean("skip-dep-check").pipe(
  Options.withDescription(
    "Skip dependency inbox checks for manual recovery or forced re-entry",
  ),
  Options.withDefault(false),
);
...
const planCmd = Command.make(
  ...
  ({
    ...
    Effect.gen(function* () {
      ...
      const stagesFromText =
        stagesFrom._tag === "Some" ? stagesFrom.value : undefined;
      const writePlanText =
        writePlan._tag === "Some" ? writePlan.value : undefined;
      const presetValue = preset._tag === "Some" ? preset.value : undefined;
      const explicitStagesEither = stagesFromText
        ? yield* Effect.try({
            try: () => parseAndValidateStagesFile(stagesFromText),
            catch: (error) =>
              error instanceof Error ? error : new Error(String(error)),
          }).pipe(Effect.either)
        : undefined;

      if (explicitStagesEither?._tag === "Left") {
        yield* Console.log(
          respondError(
            "workload plan",
            explicitStagesEither.left.message,
            "WORKLOAD_PLAN_INVALID_STAGES_FILE",
            "Pass a valid explicit stage JSON file or omit --stages-from to use autogenerated stages",
            [
              {
                command:
                  "workload plan <intent> [--repo <repo>] [--stages-from <path>]",
                description: "Retry with a valid explicit stage file",
                params: {
                  intent: {
                  ...
                  "stages-from": {
                    description: "Path to the explicit stages JSON file",
                    value: stagesFromText,
                  },
                },
              },
            ],
      ...
      const explicitStages = explicitStagesEither?.right;
      const effectiveShape =
        explicitStages && shape === "auto"
          ? explicitStages.dagInfo.inferredShape
          : shape;
      ...
      const stages = explicitStages
        ? convertToWorkloadStages(explicitStages.stages)
        : baseResult.plan.stages;
      const stageAcceptanceVerification = explicitStages
        ? Array.from(
            new Set(stages.flatMap((stageEntry) => stageEntry.verification)),
          )
        : [];
      const planShape = effectiveShape;
      const shapeReason = explicitStages
        ? shape === "auto"
          ? `Shape inferred from explicit stage DAG in ${stagesFromText}: ${explicitStages.dagInfo.inferredShape}`
          : `Shape pinned by caller with --shape ${shape}; explicit stage DAG would infer ${explicitStages.dagInfo.inferredShape}`
        : baseResult.inference.shape.reason;
      const shapeInferred = explicitStages ? shape === "auto" : baseResult.inference.shape.inferred;
      const warnings = explicitStages
        ? Array.from(
            new Set([...baseResult.warnings, ...explicitStages.warnings]),
          )
        : baseResult.warnings;
      const resultBase: WorkloadPlanningResult = explicitStages
        ? {
            ...baseResult,
            request: {
              ...baseResult.request,
            ...
            plan: {
              ...baseResult.plan,
              shape: planShape,
              summary: `${planShape} ${baseResult.plan.kind} planned for ${baseResult.plan.mode} execution on ${baseResult.plan.backend}`,
              why: [
                ...baseResult.plan.why,
                `explicit stages loaded from ${stagesFromText}`,
                `DAG inferred shape: ${explicitStages.dagInfo.inferredShape}`,
              ],
              stages,
              verification: Array.from(
                new Set([
                  ...baseResult.plan.verification,
                  ...stageAcceptanceVerification,
                ]),
              ),
            },
            metadata: {
              ...
              stagesFile: stagesFromText,
              dagInfo: toSerializableDagInfo(explicitStages.dagInfo),
            },
            inference: {
              ...baseResult.inference,
              shape: {
                value: planShape,
                inferred: shapeInferred,
                reason: shapeReason,
              },
            },
            warnings,
      ...
      const artifactPath = writePlanText
        ? resolvePlanArtifactPath(writePlanText, resultBase.plan.workloadId)
        : undefined;
        ...
        ? {
            ...
            plan: {
              ...
              next_actions: buildPlanLevelActions(
                normalizedInput,
                resultBase.plan.workloadId,
                {
                  value: resultBase.inference.target.value,
                  inferred: resultBase.inference.target.inferred,
                  localRepo: resultBase.inference.target.localRepo,
                  warnings: [],
                  scope: resultBase.inference.target.scope,
                },
                resultBase.inference.kind,
                resultBase.inference.shape,
                resultBase.inference.mode,
                true,
              ),
            },
          }
      ...
      const attachStagesFromToActions = (
        actions: Array<{
          command: string;
          description: string;
        ...
        explicitStages && stagesFromText
          ? actions.map((action) =>
              /(?:^|\s)workload plan\b/u.test(action.command) &&
                !action.command.includes("--stages-from")
                ? {
                    ...action,
                    command: `${action.command} --stages-from ${shellQuote(stagesFromText)}`,
                    params: {
                      ...(action.params ?? {}),
                      "stages-from": {
                        description: "Path to the explicit stages JSON file",
      ...
      const hydratedResult: WorkloadPlanningResult = explicitStages
        ? {
            ...result,
            plan: {
              ...result.plan,
              next_actions: attachStagesFromToActions(result.plan.next_actions),
            },
          }
        : result;

      const nextActions = attachStagesFromToActions(
        buildPlanNextActions(normalizedInput, hydratedResult),
      );
      ...
      yield* Console.log(respond("workload plan", hydratedResult, nextActions));
    }),
).pipe(
...
const runCmd = Command.make(
  "run",
  {
    ...
    skipDepCheck: runSkipDepCheckOption,
  },
  ({
    planArtifact,
    stage,
    ...
    Effect.gen(function* () {
      ...
      const parsed = parsedEither.right;
      const selectedStageEither = yield* Effect.try({
        try: () => selectDispatchStage(parsed.result, stageId),
        catch: (error) =>
          error instanceof Error ? error : new Error(String(error)),
      }).pipe(Effect.either);

      if (selectedStageEither._tag === "Left") {
        yield* Console.log(
          respondError(
            "workload run",
            selectedStageEither.left.message,
            "WORKLOAD_RUN_INVALID_STAGE",
            "Choose a valid stage id from the saved plan and retry",
            [],
          ),
        );
      ...
      const selectedStage = selectedStageEither.right;

      if (!skipDepCheck && (selectedStage.dependsOn?.length ?? 0) > 0) {
        const dependencyCheck = inspectWorkloadStageDependencies({
          workflowId: parsed.result.plan.workloadId,
          stage: selectedStage,
        });

        if (
          dependencyCheck.pending.length > 0 ||
          dependencyCheck.failed.length > 0
        ) {
          const formatDependencyLine = (dependency: {
            ...
            `${dependency.dependencyId} (${dependency.status}${dependency.requestId ? ` via ${dependency.requestId}` : ""}${dependency.error ? `: ${dependency.error}` : ""})`;

          const detailParts = [
            dependencyCheck.satisfied.length > 0
              ? `satisfied: ${dependencyCheck.satisfied.map(formatDependencyLine).join(", ")}`
              : undefined,
            dependencyCheck.pending.length > 0
              ? `pending: ${dependencyCheck.pending.map(formatDependencyLine).join(", ")}`
              : undefined,
            dependencyCheck.failed.length > 0
              ? `failed: ${dependencyCheck.failed.map(formatDependencyLine).join(", ")}`
              : undefined,
          ].filter(Boolean);
          ...
          yield* Console.log(
            respondError(
              "workload run",
              `Stage ${selectedStage.id} is blocked by dependency state. ${detailParts.join(" · ")}`,
              "WORKLOAD_RUN_DEPENDENCIES_INCOMPLETE",
              "Wait for dependencies to complete successfully, rerun failed dependency stages, or pass --skip-dep-check only for deliberate manual recovery",
              [
                {
                  command:
                    "workload run <plan-artifact> --stage <stage-id> --skip-dep-check [--tool <tool>] [--execution-mode <mode>]",
                  description:
                    "Override dependency gating only when you are intentionally recovering or replaying the stage manually",
                  params: {
                    "plan-artifact": {
                      description: "Path to a workload plan envelope",
                      value: parsed.absolutePath,
  ...
  Console.log(
    respond(
      "workload",
      {
        ...
        shipped: {
          plan: "joelclaw workload plan <intent> [--preset docs-truth|research-compare|refactor-handoff] [--kind auto|repo.patch|repo.refactor|repo.docs|repo.review|research.spike|runtime.proof|cross-repo.integration] [--shape auto|serial|parallel|chained] [--paths-from status|head|recent:<n>] [--stages-from <path>] [--write-plan <path>]",
          dispatch:
            "joelclaw workload dispatch <plan-artifact> [--stage <stage-id>] [--to <to>] [--from <from>] [--send-mail] [--write-dispatch <path>]",
          run:
            "joelclaw workload run <plan-artifact> [--stage <stage-id>] [--tool pi|codex|claude] [--execution-mode auto|host|sandbox] [--sandbox-backend local|k8s] [--skip-dep-check] [--dry-run]",
          sandboxes:
      ...
      [
        ...
        {
          command: `joelclaw workload plan ${shellQuote("stage the Firecracker rollout explicitly")} --repo ${shellQuote("/Users/joel/Code/joelhooks/joelclaw")} --stages-from ${shellQuote("infra/firecracker/stages.json")} --write-plan ${shellQuote("~/.joelclaw/workloads/")}`,
          description:
            "Plan a complex project from an explicit stage DAG JSON file instead of autogenerated stage templates",
        },
        {
