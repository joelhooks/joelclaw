...
const modelsCmd = Command.make(
  "models",
  { list: modelsList, plain: modelsPlain },
  ({ list, plain }) =>
    Effect.gen(function* () {
      ...
      if (!shouldList) {
        yield* Console.log(
          respond(
            ...
            [
              {
                command: "joelclaw models --list",
                description: "List allowed gateway model IDs",
              },
              {
                command: "joelclaw models --list --plain",
                description: "Emit newline-delimited model IDs for scripts",
              },
            ],
            true,
          ),
  ...
  Effect.gen(function* () {
    ...
    yield* Console.log(
      respond(
        ...
        {
          description:
            "joelclaw — Inngest CLI for agents (HATEOAS JSON, Effect)",
          health: checks,
          functions: fns.map((f) => ({
            name: f.name,
          ...
          commands: {
            ...
            workload:
              "joelclaw workload plan <intent> [--kind <kind>] [--shape <shape>] [--autonomy <autonomy>] [--proof <proof>]",
            ...
            inngest:
              "joelclaw inngest {status|workers|register|restart-worker|reconcile|memory-e2e|memory-weekly|memory-gate|memory-schema-reconcile|memory-health}",
        ...
        [
          ...
          },
          {
            ...
            description:
              "Plan repo/coding work with the ADR-0217 workload schema",
          },
          {
            command: "joelclaw capabilities",
            description: "Discover goal-oriented command flows",
          },
          {
            command: "joelclaw recover list",
            description: "List deterministic recovery runbooks",
          },
          {
            command: "joelclaw deploy worker",
            description: "Preview worker deploy sync (dry-run)",
          },
          {
            command: "joelclaw heal list",
            description: "List deterministic heal runbooks",
          },
          {
            command: "joelclaw loop status",
            description: "Active loop status (Redis)",
          },
          { command: "joelclaw runs", description: "List recent runs" },
          {
            command: "joelclaw secrets status",
            description: "Check secrets backend capability health",
          },
          {
            command:
          ...
          },
          {
            ...
            description: "Event types and payloads",
          },
          {
            command: "joelclaw skills audit",
            description: "Run on-demand skill garden audit",
          },
        ],
