    ...
    return emitMeasuredOtelEvent(
      ...
      async () => {
        ...
        await step.run("copy-snapshot-to-host", async () => {
          await runShell(
            `kubectl cp -n ${TYPESENSE_NAMESPACE} ${TYPESENSE_POD}:${snapshotPath} ${TYPESENSE_STAGE_ROOT}`,
            $`kubectl cp -n ${TYPESENSE_NAMESPACE} ${TYPESENSE_POD}:${snapshotPath} ${TYPESENSE_STAGE_ROOT}`.quiet().nothrow()
          );
        });
