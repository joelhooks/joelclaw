  ...
  async ({ event, step }) => {
    ...
    const synthesized = await step.run(
      ...
      async (): Promise<{ markdown: string; usage?: Record<string, unknown> }> => {
        const openRouterKey = leaseSecret("openrouter_api_key", "1h");
        ...
        const rawBody = await response.text();
        if (!response.ok) {
          throw new Error(`OpenRouter failed (${response.status}): ${rawBody.slice(0, 800)}`);
        }

        const parsed = parseJsonFromText(rawBody) as OpenRouterResponse | null;
        if (!parsed) {
          throw new Error("OpenRouter returned non-JSON response");
        }

        const markdown = extractOpenRouterContent(parsed);
        if (!markdown) {
          throw new Error("OpenRouter returned empty synthesis output");
        }
