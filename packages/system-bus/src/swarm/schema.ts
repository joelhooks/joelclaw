// Ported from can1357/oh-my-pi (MIT licensed). Credit: Can Boluk (@can1357)
...
export type SwarmTool = "codex" | "claude" | "pi";
export type SwarmSandbox = "workspace-write" | "danger-full-access";

const DEFAULT_TOOL: SwarmTool = "codex";
const DEFAULT_SANDBOX: SwarmSandbox = "workspace-write";
...
const VALID_TOOLS = new Set<string>(["codex", "claude", "pi"]);
const VALID_SANDBOXES = new Set<string>(["workspace-write", "danger-full-access"]);

function parseTool(value: string | undefined, fieldName: string): SwarmTool {
  const candidate = value?.trim() ?? DEFAULT_TOOL;
  if (!VALID_TOOLS.has(candidate)) {
    throw new Error(
      `${fieldName} must be one of: ${[...VALID_TOOLS].join(", ")}`,
    );
  }
  return candidate as SwarmTool;
}

function parseSandbox(value: string | undefined, fieldName: string): SwarmSandbox {
  const candidate = value?.trim() ?? DEFAULT_SANDBOX;
  if (!VALID_SANDBOXES.has(candidate)) {
    throw new Error(
      `${fieldName} must be one of: ${[...VALID_SANDBOXES].join(", ")}`,
    );
  }
  return candidate as SwarmSandbox;
}
...
export function parseSwarmYaml(content: string): SwarmDefinition {
  ...
  if (
    !swarm.agents
    || typeof swarm.agents !== "object"
    || Object.keys(swarm.agents).length === 0
  ) {
  ...
  const globalTool = parseTool(swarm.tool, "swarm.tool");
  ...
  for (const [name, config] of Object.entries(swarm.agents)) {
    ...
    agents.set(name, {
      ...
      tool: parseTool(config.tool ?? globalTool, `Agent '${name}' tool`),
      sandbox: parseSandbox(config.sandbox, `Agent '${name}' sandbox`),
    });
  }
