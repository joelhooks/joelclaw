...
Search both Central Typesense and raw flagg Pi sessions from Panda:

```bash
joelclaw sessions search "<query>" \
  --source both \
  --machine flagg \
  --ssh-target joel@flagg \
  --limit 8
```
...
```bash
joelclaw sessions search "<query>" --source both --machine flagg --limit 5 --extract
# singular alias also works:
joelclaw session search "<query>" --source both --machine flagg --limit 5 --extract
```
...
```bash
joelclaw sessions chunks "<query>" --source local --machine flagg --limit 20 --context-before 2 --context-after 4
```
...
```bash
joelclaw sessions signals --kind friction --source local --machine flagg --since 14d --limit 20
joelclaw sessions signals --kind any --source local --machine flagg --since 14d --sample 20 --review-out ~/.joelclaw/session-signals/review.jsonl --evaluate
joelclaw sessions signals --kind mode-mismatch --source local --machine flagg --since 14d --evaluate
joelclaw sessions signals --kind any --source local --machine flagg --since 8w --format ndjson | jq -c 'select(.type=="hit" and .improvement.reviewPriority=="high")'
joelclaw sessions friction --source local --machine flagg --since 14d
```
...
```bash
joelclaw sessions search "<query>" --source typesense --machine flagg --runtime pi --limit 8
joelclaw sessions search "<query>" --source typesense --machine flagg --runtime codex --limit 8
joelclaw sessions search "<query>" --source typesense --machine all --runtime all --limit 8
```

Raw local Pi sessions only, for example on flagg itself:

```bash
joelclaw sessions search "<query>" --source local --machine flagg --limit 8
```

Raw remote Pi sessions only:

```bash
joelclaw sessions search "<query>" --source ssh --ssh-target joel@flagg --limit 8
```
...
- `ssh joel@flagg` access

Smoke test:

```bash
ssh joel@flagg 'hostname && python3 --version && find ~/.pi/agent/sessions -type f -name "*.jsonl" | head'
```
...
With `--source both`, the raw side uses `local` when the current hostname matches `--machine`; otherwise it uses `ssh`. That keeps flagg from SSHing to itself. If the local Machine has no Typesense credential, `both` skips Typesense and returns raw local hits with `typesenseSkipped`; use `--source local` when you want that explicitly. If Typesense is unavailable in `both` mode, the CLI reports `typesenseUnavailable` and continues with raw results.
