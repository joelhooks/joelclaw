...
- **Execution mode: host vs sandbox (ADR-0217 Story 4).** `system/agent-dispatch` accepts `executionMode: "host" | "sandbox"` (default: `"host"`). Host mode uses the existing shared-checkout path. Sandbox mode (not yet fully implemented) will route to isolated k8s Job runners. The execution mode is captured in the `InboxResult.executionMode` field for observability. Set via `PRD_EXECUTION_MODE` environment variable in `trigger-prd.ts`.
