...
- **Execution mode: host vs sandbox (ADR-0217 Story 4).** `system/agent-dispatch` accepts `executionMode: "host" | "sandbox"` (default: `"host"`). Host mode uses the existing shared-checkout path. Sandbox mode routing is under development through incremental proof gates. **Gate A** (non-coding vertical slice) is proven via `packages/agent-execution/__tests__/gate-a-smoke.test.ts` — proves truthful state transitions (`running` → `completed`), artifact generation, zero host dirt, failure handling, and JSON serialization using a local executor. **Gate B** (minimal coding sandbox) is proven via `packages/agent-execution/__tests__/gate-b-smoke.test.ts` — proves repo materialization, real git operations (clone, checkout, add, commit, format-patch), patch artifact export, verification capture, and zero host dirt using a local executor with real git. Gate C (k8s Job launcher) is next. See `packages/restate/README.md` for full gate status. The execution mode is captured in the `InboxResult.executionMode` field for observability. Set via `PRD_EXECUTION_MODE` environment variable in `trigger-prd.ts`.
