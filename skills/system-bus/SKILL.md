...
For ADR-0217 Phase 3 Story 2-3, the operator surfaces are `joelclaw queue observe`, `joelclaw queue pause`, `joelclaw queue resume`, and `joelclaw queue control status`. `queue observe` still answers “what would Sonnet do right now?” in dry-run, but its `control` block now reflects the shipped deterministic control plane: active pauses, expirations, and recent `queue.control.*` OTEL. `queue pause` / `queue resume` are the bounded manual apply path before any automatic observer mutation. `queue control status` is the direct operator truth source for active manual controls and recent `queue.control.applied|expired|rejected` events.
