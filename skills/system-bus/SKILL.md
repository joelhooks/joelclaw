...
For ADR-0217 Phase 3 Story 2, the dry-run operator surface is `joelclaw queue observe`. It builds a live queue snapshot from current queue state plus recent `queue.dispatch.*` and `queue.triage.*` OTEL, runs the bounded Sonnet observer in `dry-run` only, and returns the current `snapshot`, the current `decision`, recent `queue.observe.*` history, and an explicit `control` block that says the deterministic queue-control plane is not shipped yet. If the installed CLI cannot answer “what would Sonnet do right now?” from that one command, Story 2 is not actually done.
