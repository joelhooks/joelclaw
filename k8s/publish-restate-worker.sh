...
MANIFEST="${MANIFEST:-$ROOT_DIR/k8s/restate-worker.yaml}"
NAMESPACE="${NAMESPACE:-joelclaw}"
DEPLOYMENT="${DEPLOYMENT:-restate-worker}"
REGISTRY="${REGISTRY:-ghcr.io}"
OWNER="${OWNER:-joelhooks}"
IMAGE_NAME="${IMAGE_NAME:-restate-worker}"
DOCKERFILE="${DOCKERFILE:-$ROOT_DIR/packages/restate/Dockerfile}"
...
if [[ -z "$ghcr_token" ]] && command -v secrets >/dev/null 2>&1; then
  if ghcr_token="$(secrets lease ghcr_pat --ttl 20m --client-id publish-restate-worker 2>/dev/null)"; then
...
if ! grep -q "ghcr.io/.*/restate-worker:" "$MANIFEST"; then
  echo "Could not find restate-worker image reference in $MANIFEST" >&2
  exit 1
fi
sed -i.bak -E "s|image: ghcr.io/.*/restate-worker:[^[:space:]]+|image: $IMAGE|" "$MANIFEST"
rm -f "$MANIFEST.bak"
...
kubectl -n "$NAMESPACE" exec "$POD" -- sh -lc 'curl -fsS http://127.0.0.1:9080/' || true
