...
**Rhizomatic Memory Service**:
A Central-owned memory substrate for agent beliefs, where claims are signed, append-only, and resolved through per-reader trust policy.
_Avoid_: MCP server, vector store, scratchpad, second brain

**Native Pi Memory Surface**:
The Pi extension/tool layer that exposes Rhizomatic Memory Service primitives directly inside Pi sessions, with MCP treated as a compatibility adapter for non-Pi clients.
_Avoid_: MCP wrapper, proxy tool, partial memory shim

**Rhizomatic Service Endpoint**:
The configured base URL a client uses to reach a Rhizomatic Memory Service over the Network.
_Avoid_: hardcoded tailnet URL, host-specific constant, topology assumption

**Rhizomatic Adapter Contract**:
The adapter-facing HTTP contract in `joelhooks/pi-rhizomatic`, useful for local/dev reference services and generic runtime hooks. It is not Myk's upstream Rhizomatic spec or Chorus implementation.
_Avoid_: upstream spec, substrate contract, service-location API, joelclaw-only contract

**Chorus Backend**:
Myk Bilokonsky's upstream Chorus/Rhizomatic HTTP MCP server, used as the real backend for signed authors, policy lenses, contested facts, and future federation.
_Avoid_: copied implementation, fake reference service, joelclaw-owned substrate

**Network Memory Coverage**:
The joelclaw goal that Pi, Claude, Codex, and future agent runtimes can all use the same Rhizomatic Memory Service through their native runtime surfaces.
_Avoid_: Pi-only memory, MCP-only bridge, local demo

**Canary Memory Store**:
A resettable experimental Rhizomatic store used for Pi canaries before any memory is promoted to durable production belief memory.
_Avoid_: production memory, source of truth, sacred log

Current canary state, 2026-06-13/14: public repo `https://github.com/joelhooks/pi-rhizomatic` is created and `main` points at `a09a7473f13d914e516d64ed65db43e49f5e6243`; the package is now a Pi/Claude/Codex adapter that can use Myk Bilokonsky's upstream Chorus HTTP MCP server via `backend: "chorus-http"`. Flagg runs upstream `mbilokonsky/rhizomatic` at `/Users/Shared/joelclaw/upstream/rhizomatic` under launchd service `com.joelclaw.chorus-rhizomatic`, listening on loopback `127.0.0.1:4821` and writing signed Chorus deltas to `/Users/Shared/joelclaw/services/rhizomatic/chorus-memory.jsonl`. Blaine and Panda use launchd SSH tunnels from local `127.0.0.1:7331` to Flagg `127.0.0.1:4821`; Flagg clients talk directly to `127.0.0.1:4821/mcp`. Legacy fake reference service `com.joelclaw.rhizomatic` is disabled. Proof label `chorus-adapter-20260614T041223Z-{blaine,flagg,panda}` showed all three machines can use Pi/Claude/Codex-style canary calls against the same Chorus store.
...
- **Central** may own a **Rhizomatic Memory Service** for agent belief memory
- A **Machine** running Pi may use a **Native Pi Memory Surface** to read and write the **Rhizomatic Memory Service** through a configured **Rhizomatic Service Endpoint**
- A **Native Pi Memory Surface** owns the **Rhizomatic Adapter Contract** and depends on no deployment topology
- A **Rhizomatic Memory Service** may be backed by a **Chorus Backend** through the adapter
- A **Rhizomatic Memory Service** should provide **Network Memory Coverage** across runtime-native surfaces
- A **Rhizomatic Memory Service** may start with a **Canary Memory Store** before promotion
...
5c. **New Central-owned services start on Flagg.** While Panda remains the current Central for legacy runtime, new Central-owned services should be born on Flagg under the joelclaw service identity and launchd/reboot contract. This does not create active/active Central: a new service may be flagg-hosted before whole-Central cutover only if its authoritative state and routing are explicitly scoped, reversible, and documented. See `docs/decisions/0247-new-central-services-start-on-flagg.md`.
...
8d. **Native Pi gets full Rhizomatic primitive parity.** The **Native Pi Memory Surface** should expose all Rhizomatic/Chorus primitives as first-class Pi tools/commands rather than hiding advanced operations behind MCP or a partial shim. During the experiment, primitives do not need per-tool confirmation gates; the boundary is explicit canary scope, clear tool descriptions, resettable experimental state, and promotion only after evidence.
8e. **Rhizomatic starts as a resettable canary store.** The first flagg-hosted Rhizomatic service uses a **Canary Memory Store** for Pi canaries, not production joelclaw memory/search. It may be backed up for debugging, but it is allowed to be wiped. Promotion to durable production belief memory requires a later explicit decision.
8f. **Rhizomatic transport uses configured HTTP, not hardcoded topology.** Native Pi talks to the Rhizomatic Memory Service over ordinary HTTP JSON, but the **Rhizomatic Service Endpoint** is configuration, not source code. Flagg/Tailscale is Joel's deployment choice, not a package assumption; the package must support another base URL for another Network without patching code.
8g. **Rhizomatic endpoint config is layered and explicit.** Native Pi resolves the **Rhizomatic Service Endpoint** from `RHIZOMATIC_SERVICE_URL`, then project `.pi/rhizomatic.json`, then user `~/.pi/agent/rhizomatic.json`. There is no baked-in network default. If unset, the tools return a not-configured result with setup instructions. Auth material is referenced through env/agent-secrets, not stored as plaintext config.
8h. **Pi package depends on configured endpoints, not service location.** The native Pi package must not care whether the memory backend is on Flagg, Panda, localhost, Tailscale Serve, or some future host. It only requires a configured endpoint and backend mode such as `chorus-http`.
8i. **The native Pi package owns an adapter contract, not Myk's substrate.** The **Rhizomatic Adapter Contract** is authored as Effect schemas in the native Pi package for local/dev reference services and generated OpenAPI. Real Rhizomatic semantics come from the upstream **Chorus Backend**.
8j. **Keep the Rhizomatic Pi package whole in v0.** The v0 native Pi package contains the extension, Effect schemas, generated OpenAPI, and HTTP client helpers together. Split a standalone contract/client package only after real non-Pi consumers make that useful.
8k. **The portable package is public `joelhooks/pi-rhizomatic`.** The native Pi package should live in the public `joelhooks/pi-rhizomatic` repo and avoid joelclaw-specific assumptions. joelclaw remains a consumer/service implementation for one extremely specific nerd's Network.
8l. **Public Rhizomatic package keeps deployment details configurable and private.** `joelhooks/pi-rhizomatic` may ship generic user/system/local/bridge config support, but it must not contain Flagg/Panda/Blaine names, Joel's tailnet topology, joelclaw service-account paths, Slack thread IDs, or private joelclaw domain context. Joel-specific endpoint/auth/bridge config lives in local config, private joelclaw docs, or agent-secrets.
8m. **Public Rhizomatic package ships a local reference service.** `joelhooks/pi-rhizomatic` should include a tiny local HTTP service, e.g. `pi-rhizomatic serve --store ./rhizomatic.jsonl`, that implements the package-owned contract for localhost testing and non-joelclaw users. The reference service is for local testing, not Joel's Flagg canary.
8n. **Flagg Rhizomatic targets Network Memory Coverage.** Joel's Flagg canary should run the upstream **Chorus Backend** through private joelclaw launchd/tunnel wiring. The first Network canary is not successful until Pi, Claude, and Codex can all begin, brief, remember, and end against the same Flagg **Canary Memory Store** through their runtime-native surfaces. A localhost reference service or Pi-only demo is not enough.
8o. **Runtime hook helpers are public and generic.** `joelhooks/pi-rhizomatic` should ship generic Claude/Codex hook helpers and CLI commands that speak either the **Rhizomatic Adapter Contract** or the `chorus-http` backend. Private joelclaw config decides where those hooks point, how they are installed, and which canary store they use.
8p. **`pi-rhizomatic init` is the setup front door.** The public package should provide an init command that inspects Pi/Claude/Codex config, previews proposed endpoint/config/hook changes, and writes only with confirmation. It defaults to dry-run preview; mutation requires `--write` or an explicit interactive confirmation. Joel-specific overlays may feed it private config, but the public command stays generic.
8q. **Rhizomatic v0 auth is optional bearer auth.** The local reference service binds `127.0.0.1` and runs without auth by default. When exposed beyond localhost, users can configure bearer auth through an environment variable such as `RHIZOMATIC_TOKEN`; clients reference tokens through env or secret names, not plaintext config. Joel's Flagg service always uses private tokened access.
8r. **Native Pi Rhizomatic names are boring and explicit.** Pi tools use the `rhizomatic_` prefix, e.g. `rhizomatic_begin_session` and `rhizomatic_briefing`; the CLI is `pi-rhizomatic`; slash commands use `/rhizomatic ...`.
8s. **Rhizomatic runtime lifecycle is automatic and non-blocking.** Pi/Claude/Codex native surfaces should automatically call `begin-session` and `briefing` at session start, then `end-session` at stop when the runtime exposes the lifecycle. Writes must be idempotent by runtime/session/event key. Service unavailability should warn or queue when easy, but not block normal agent use during the canary.
8t. **Rhizomatic queues failed writes, not reads.** Mutating Rhizomatic calls may fall back to a local JSON outbox such as `~/.rhizomatic/outbox/`, drained by `pi-rhizomatic drain`. Read calls fail fast with a useful warning; they are not queued.
8u. **Rhizomatic Network canary proof requires cross-runtime recall.** The first full Network canary starts Flagg against an empty **Canary Memory Store**, has Pi, Claude, and Codex each write runtime-specific canary claims, then proves each runtime can recall all three claims and that all three sessions have attributed `begin-session` and `end-session` records in the same store.
8v. **Build the portable contract/package before the Flagg implementation.** `joelhooks/pi-rhizomatic` should define the Effect schemas, generated OpenAPI, local reference service, Pi extension, and generic Claude/Codex hook helpers before joelclaw implements the Flagg service against that contract. This prevents Joel-specific infrastructure from shaping the portable API by accident.
...
- **"New services on Flagg while Panda remains Central"** — resolved: new Central-owned services should start on Flagg under a service identity, with scoped authority and explicit routing, without treating Panda and Flagg as active/active Central peers.
- **"Dangerous Rhizomatic primitives"** — resolved: for the experiment, expose all primitives without native confirmation gates; treat mistakes as canary-state pollution, not a security boundary failure.
- **"First Rhizomatic store authority"** — resolved: use a resettable **Canary Memory Store** on Flagg; do not ingest it into production joelclaw memory/search until a later promotion decision.
- **"Rhizomatic service URL"** — resolved: use a configured **Rhizomatic Service Endpoint**; do not hardcode Joel's Tailscale topology into the Pi package.
- **"Pi package coupling to Flagg"** — resolved: the native Pi package depends on configured endpoint/backend mode, not the host, Tailscale shape, or deployment mechanism.
- **"Rhizomatic contract source of truth"** — resolved: the native Pi package owns only the adapter-facing contract; upstream Rhizomatic/Chorus remains Myk's substrate and implementation.
- **"joelclaw as Rhizomatic owner"** — resolved: joelclaw is Joel-specific infrastructure; it hosts Joel's service implementation but does not own the portable Rhizomatic contract.
- **"Rhizomatic contract package split"** — resolved: keep extension, Effect schemas, generated OpenAPI, and HTTP client helpers in one native Pi package for v0; split later only when useful.
- **"Rhizomatic package public identity"** — resolved: use public repo `joelhooks/pi-rhizomatic`; keep joelclaw as a specific service implementation/consumer, not the portable package's worldview.
- **"Public package config boundary"** — resolved: public package may support generic user/system/local/bridge config, but Joel-specific hostnames, paths, Slack thread IDs, service accounts, and private context stay out of the public repo.
- **"Rhizomatic local reference service"** — resolved: public package should include a tiny local HTTP reference service implementing the same contract for localhost testing and non-joelclaw users, but Joel's Flagg canary uses a joelclaw-specific full-network implementation.
- **"Flagg Rhizomatic canary scope"** — resolved: first success requires **Network Memory Coverage**: Pi, Claude, and Codex all begin, brief, remember, and end against the same Flagg **Canary Memory Store** through native surfaces. Pi-only is not enough.
- **"Claude/Codex adapter placement"** — resolved: public `joelhooks/pi-rhizomatic` ships generic hook helpers; private joelclaw config handles endpoint, install, service identity, and canary wiring.
- **"Rhizomatic install story"** — resolved: `pi-rhizomatic init` is the setup front door; it previews and confirms Pi/Claude/Codex config and hook changes instead of requiring manual wiring.
- **"Rhizomatic v0 auth"** — resolved: localhost reference service is open by default; exposed services use optional bearer auth via env/secret references, and Joel's Flagg deployment is always tokened.
- **"Rhizomatic tool naming"** — resolved: use explicit `rhizomatic_` prefix for Pi tools, `pi-rhizomatic` for CLI, and `/rhizomatic ...` for slash commands.
- **"Rhizomatic lifecycle automation"** — resolved: runtime-native hooks automatically begin/brief at start and end-session at stop; calls are idempotent and non-blocking during canary outages.
- **"Rhizomatic offline behavior"** — resolved: failed mutating calls queue to a local outbox and reads fail fast; `pi-rhizomatic drain` retries queued writes.
- **"Rhizomatic Network canary proof"** — resolved: first success requires Pi, Claude, and Codex each writing canary claims, each recalling all three, and the shared store showing attributed begin/end records for all three runtimes.
- **"Rhizomatic implementation order"** — resolved: build the portable public package/contract first, then make joelclaw's Flagg service conform to it.
