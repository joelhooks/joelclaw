...
10g. **Shelf-local NAS mounts are LAN-shaped.** `three-body` may resolve through Tailscale/MagicDNS for SSH/admin/remote access, but Central storage mounts on Flagg use the NAS LAN IP (`192.168.1.163`) over `en0`, not the `three-body` hostname. The ASUSTOR NFS exports are LAN-scoped, so using MagicDNS can make the client source a tailnet IP and produce misleading NFS permission failures. Tailscale is the control/fallback plane, not the default data plane for mounted storage on the same shelf.

10h. **Health must be vision-aligned, not Panda nostalgia.** The health surface should report by responsibility lane: Central (Flagg/Mac Studio native launchd services and authoritative state), Relay (Panda account-bound leftovers and decommission blockers), and Satellite (Blaine/other capture clients). Panda k8s/PDS failures are real migration/decommission work, but they must not be presented as the whole joelclaw system once Flagg owns current Central duties. Health output should include concrete migration TODOs and distinguish current authority from legacy surfaces.
