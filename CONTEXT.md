...
**Relay Sandbox**:
An isolated per-User operating-system boundary on a Relay Machine that hosts that User's Channel Accounts and relay processes.
_Avoid_: shared login, family account, container
...
8b. **iMessage relay isolation is per Relay Sandbox.** Panda's iMessage relay uses one Relay Sandbox per User so iCloud session state, Messages databases, Keychain entries, TCC/FDA grants, and relay launchd agents do not cross User boundaries.
...
- **"Panda user account"** — resolved: use **Relay Sandbox** for the per-User isolation boundary on Panda; macOS user accounts are the likely implementation for iMessage but not the domain term.
