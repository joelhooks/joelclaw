...
function toAdrHref(target: string): string {
  const normalized = stripMarkdownExtension(target)
    .replace(/\\/g, "/")
    .replace(/^\.?\//, "")
    .trim();
...
function toImageHref(target: string): string {
  const normalized = stripMarkdownExtension(target)
    .replace(/\\/g, "/")
    .replace(/^\.?\//, "")
    .trim();
...
/**
 * Convert Obsidian wikilinks/embeds to standard markdown nodes for MDX.
 *
 * Examples:
 * - ![[adr/foo.jpg|Alt]] -> ![Alt](/images/adr/foo.jpg)
 * - [[0077-memory-system-next-phase]] -> [0077-memory-system-next-phase](/adrs/0077-memory-system-next-phase)
 * - ![[0077-memory-system-next-phase]] -> [0077-memory-system-next-phase](/adrs/0077-memory-system-next-phase)
 */
