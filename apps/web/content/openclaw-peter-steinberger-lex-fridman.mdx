...
Around the 2:34 mark, when Lex asks Peter about OpenClaw's architecture — the gateway, the harness, the agentic loop — Peter drops a line that caught my attention:

> Everybody should implement an agent loop at some point in their lives. Because it's like the Hello World of AI. And it's actually quite simple.

This is a deceptively important idea. In traditional programming, "Hello World" isn't about printing text to a screen. It's the moment you prove the entire toolchain works — compiler, runtime, I/O — and you understand the shape of the system. The agent loop is the same thing for the AI era: you send a message, the model reasons about it, it calls a tool, you feed the result back, it reasons again, and eventually it gives you an answer. That's it. That's the whole pattern.

Peter even did this as a workshop exercise at a conference in Paris — teaching people to build their own mini Claude Code. His point: once you see how the loop works, the "magic" dissolves. It's not magic. It's a while loop with function calls.

The claim that keeps echoing: **if you understand the agent loop, you understand the entire paradigm shift**. Everything else — skills, memory, heartbeats, self-modification, MCP debates — is detail layered on top of that core loop. The same way every web framework is ultimately request → process → response, every AI agent is ultimately prompt → reason → act → observe → repeat.

What makes this framing powerful is the implicit challenge: if it's so simple, why haven't you built one? The answer for most people is that the gap between "I use ChatGPT" and "I have an agent running on my machine" feels enormous, but it's not. It's maybe [50 lines of Python](https://www.reddit.com/r/ChatGPTCoding/comments/164ughh/a_tiny_autonomous_agent_i_made_in_25_lines_of/) or a weekend project with any LLM SDK. [Boot.dev](https://www.freecodecamp.org/news/build-an-ai-coding-agent-with-python-and-gemini/), [Siddharth Bharath](https://www.siddharthbharath.com/build-a-coding-agent-python-tutorial/), [Leonie Monigatti](https://www.leoniemonigatti.com/blog/ai-agent-from-scratch-in-python.html) — there's a growing set of tutorials that walk through building a from-scratch agent in an afternoon.

The parallel to JoelClaw is direct. The system-bus worker, the gateway, the heartbeat, the memory pipeline — all of it sits on top of that same fundamental loop. The difference between a toy agent and a real one isn't the loop itself. It's what you wire up around it: persistence, memory, tools, channels, safety gates. But the core loop is still just a while loop.
...
> Everybody should implement an agent loop at some point in their lives. It's the Hello World of AI.
