...
description: "Voice as a practical interface for AI agents. The complete infrastructure for phone-based conversations — LiveKit, SIP trunks, distributed topology, and why CarPlay is the endgame."
tags: ["voice", "livekit", "sip", "infrastructure"]
---

Voice is a UI for getting shit done when typing is impractical. Driving to the office, walking the dog, cooking dinner — times when your hands are busy but your brain isn't. That's why I built phone-based voice conversations into joelclaw.

The dream is CarPlay integration: "Hey, implement that feature we discussed yesterday." The agent asks clarifying questions, you dial in the requirements during your commute, and by the time you're home there's a PR waiting. Voice for utility, not novelty.

## The Architecture

[ADR-0043](/adrs/0043-agent-voice-conversations) laid out the vision: self-hosted voice conversations via LiveKit. The core infrastructure runs on my hardware, but the voice pipeline uses best-in-class cloud services — Deepgram for STT, Claude for reasoning, ElevenLabs for TTS, Telnyx for phone connectivity. Here's what got deployed.

### Network Topology

The system spans two nodes:
...
- LiveKit server on Talos k8s cluster (ports 7880/7881)
...
Runs as a background process. Logs to `/tmp/joelclaw/voice-agent.log`.
...
**Public Mode**: Open the phone number to everyone. Different context, restricted tools — anyone can call and ask about joelclaw, browse ADRs by voice, understand the architecture. Like office hours, but 24/7 and automated.

**Write While Walking**: Start an article with voice. The agent asks clarifying questions, you refine the angle, add examples. By the end of the walk, you have a solid outline. Q&A interaction produces better writing than staring at a blank page.

**Native App + CarPlay**: The real endgame. "Start implementing that PRD from yesterday." Drive and code — request features, answer clarifying questions, get progress callbacks. Commute time becomes productive time.

Voice is just another interface to the system — same tools, same data, same capabilities. But optimized for when your hands are busy and your brain isn't.
