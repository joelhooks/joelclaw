...
But here's the thing — Docker Compose works until it doesn't. The moment I want to add a health check, or schedule a batch job, or run a service on a different machine, I'm writing shell scripts and launchd plists and hoping they don't drift apart. Kubernetes solves that. Whether three containers on a personal machine justifies Kubernetes is a question I've decided not to think too hard about.

I spiked it. The spike worked. So I kept going. The whole migration — spike, debugging, manifests, cutover — was one session. This is what happened.
