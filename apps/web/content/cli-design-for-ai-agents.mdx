...
Every response includes `next_actions` — an array of commands the agent can run next, with descriptions. The agent never has to memorize your command tree. It discovers available actions from the output of the last command it ran.
...
The `next_actions` are **contextual** — they change based on what just happened. A failed command suggests different next steps than a successful one. An error includes a `fix` field in plain language. The agent has everything it needs to self-recover.

This is Roy Fielding's [HATEOAS](https://en.wikipedia.org/wiki/HATEOAS) constraint from REST, applied to CLIs. The application state is navigable from the response itself. No out-of-band knowledge required.
...
```json
{
  ...
  "result": {
    ...
    "commands": [
      { "name": "send", "usage": "joelclaw send <event> -d '<json>'" },
...
```json
{
  ...
  "next_actions": [
    { "command": "joelclaw logs --lines 100", "description": "Show more" }
  ]
}
```
