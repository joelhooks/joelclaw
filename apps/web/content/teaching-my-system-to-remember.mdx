...
date: "2026-02-15"
description: "Every AI session on my machine starts from zero. The system knows who I am but not what happened yesterday. Here's how I'm building an observation pipeline to fix that — stolen shamelessly from four open-source projects and one very clever pattern from Mastra."
---
...
Pi (my primary coding agent) has a context window. When a session gets long enough, it compacts — summarizes older messages to make room for new ones. The summary is fine for continuing the current conversation. Goal, progress, decisions, next steps.
...
The Reflector doesn't write directly to `MEMORY.md`. That felt wrong. Memory should be curated, not auto-generated.
...
This is the piece I'm most interested in and haven't built yet.
...
I want to be honest about the status here.
...
The full design lives in [ADR-0021](https://github.com/joelhooks/joelclaw) with implementation phases, event schemas, prompts, and verification criteria. It supersedes two earlier ADRs that each tried to solve a piece of this separately.
