...
tags: ["inngest", "architecture", "durable-functions"]
---
...
Inngest server runs as a k8s StatefulSet in a [k3d cluster](/joel-deploys-k8s) with persistent volume claims. The worker is a Bun + Hono app managed by launchd with `KeepAlive: true`. Both survive reboots. Caddy terminates TLS via Tailscale certs so I can hit the dashboard from my phone.
...
Inngest published [official agent skills](https://github.com/inngest/inngest-skills) — six skills covering setup, events, steps, flow control, middleware, and durable functions. Install them and your agent knows how to build all of this without you explaining the patterns:

```bash
npx skills add inngest/inngest-skills --yes --global
```

Every pattern in this post — step-level durability, event chaining, concurrency keys, claim-check, middleware — is covered. The skills work with Claude Code, pi, Cursor, and anything that supports the [skills spec](https://agentskills.io).
