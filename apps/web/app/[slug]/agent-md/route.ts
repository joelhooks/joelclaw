...
async function toAgentMarkdown(mdxContent: string): Promise<string> {
  const result = await remark()
    ...
    .use(remarkMdLinks, { mode: "agent" }) // rewrite internal links to .agent.md endpoints
    .process(mdxContent);
...
function agentPreamble(meta: PostMeta, allPosts: PostMeta[]): string {
  ...
  const lines = [
    ...
    ``,
    `This version includes <AgentOnly> blocks with commands, configuration, and troubleshooting context that are stripped from the human HTML version.`,
    ``,
