...
system_socket_exists() {
  [[ -S "$TAILSCALE_SOCKET" ]]
}

tailscale_running() {
  tailscale_system status --json | python3 -c 'import json,sys; d=json.load(sys.stdin); assert d.get("BackendState") == "Running", d.get("BackendState"); assert d.get("Self", {}).get("Online") is True'
}

tailscale_ip_present() {
  tailscale_system ip -4 | grep -Eq '^100\.'
}
...
probe 'system tailscaled socket exists' system_socket_exists
probe 'system tailscaled backend running and online' tailscale_running
probe 'system tailscaled has IPv4 address' tailscale_ip_present
probe 'local SSH listener works' local_ssh_works
probe 'GUI Tailscale.app process not running' no_gui_app_process
probe 'GUI network extension not running' network_extension_absent_or_idle
