...
pub fn run_agents(
    ...
    let (cloud_success, cloud_output) = run_shell_with_stdin(&config.agent.cloud_command, &prompt, timeout)?;
    ...
    let (local_success, local_output) = run_shell_with_stdin(&config.agent.local_command, &prompt, timeout)?;
    if local_success {
...
pub fn run_sos(
    ...
    if osa_success {
        ...
        log::error_fields("SOS escalation failed", &[("error", truncate(&osa_output, 1000))]);
        Ok(TierOutcome::Failed)
    }
}

fn build_diagnostic_prompt(config: &Config, failed_probes: &[ProbeResult], heal_output: &str) -> String {
    ...
    prompt.push_str(&truncate(&log::tail_file(Path::new(&worker_log_path), 120), 4000));

    prompt.push_str(
        "\n\nTake action now. Execute concrete repair commands and explain what was changed. Keep it brief.",
    );
