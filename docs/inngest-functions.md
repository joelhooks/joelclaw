...
- `system/agent-dispatch` supports `executionMode: "host" | "sandbox"` (default: `"host"`). Host mode uses the existing shared-checkout path. Sandbox mode routing is under development through incremental proof gates (see `packages/restate/README.md` for gate status). **Gate A** (non-coding vertical slice) is proven via `packages/agent-execution/__tests__/gate-a-smoke.test.ts` — proves truthful state transitions, artifact generation, zero host dirt, and failure handling using a local executor. **Gate B** (minimal coding sandbox) is proven via `packages/agent-execution/__tests__/gate-b-smoke.test.ts` — proves repo materialization, real git operations, patch artifact export, verification capture, and zero host dirt using a local executor with real git. Gate C (k8s Job launcher) is next. The execution mode is captured in the `InboxResult.executionMode` field and logged for observability.
