...
- Video ingest canonical events are `pipeline/video.requested` → `pipeline/video.downloaded` + `pipeline/transcript.requested` → `pipeline/transcript.processed` + `content/summarize.requested`. The host worker also accepts legacy public aliases `pipeline/video.download`, `pipeline/transcript.process`, and `content/summarize` because older CLI/schema/skills documented those names; never let documented operator events silently produce zero function runs.
...
- `conversation-thread-enrich` vault lookup is keyword-only unless a real query vector exists. Do not use Typesense empty auto-embedding syntax like `embedding:([], k:N)` against `vault_notes`; the current schema has no compatible embedded field and fails runs with `Vector query could not find any embedded fields`. Its summarizer must fail open to a source-grounded summary from the thread messages when model JSON is empty or malformed; enrichment should not fail just because a non-critical summary model returned junk.
- `memory/batch-review` must not let LLM timeout/retry churn leave the function RUNNING/FAILED for minutes. The LLM step is a best-effort review pass with a short timeout and single fallback attempt; if output is empty or unparseable, return `status: "deferred"`, keep proposals queued, and emit `batch-review.deferred` instead of failing the run.
...
- Slack historical backfill:
  ...
  - fetches top-level channel history, expands `reply_count > 0` parents through `conversations.replies`, and also indexes bounded `search.messages` matches for the requested channel/date window because Slack search returns active replies even when the thread parent is older than the backfill window
