...
- Current honest limit: full-mode runtime startup is now earned, but a full workflow-rig stage still has not earned clean terminal completion from the agent tool itself; the compose-backed runtime comes up and the dispatch enters `execute-agent`, but the long-running stage prompt remains sticky enough that completion/cancellation truth still needs another pass
