...
- the installed CLI now has an ADR-0221 operator surface: `joelclaw workload sandboxes list|cleanup|janitor` reads the registry, reports retention/filesystem truth, supports bounded manual cleanup, and runs the dedicated expired-sandbox janitor path instead of waiting for the next sandbox startup
