...
- **Firecracker microVM primitives**: `microvm.ts` now owns Firecracker socket-path resolution, cold-boot vs snapshot-restore request builders, lifecycle helpers (`boot`, `restore`, `pause`, `snapshot`, `destroy`), and the initial workspace command-file protocol for Lambda-style single-use guest execution
- **Consumed by**: Restate workflows, system-bus functions, k8s Job launcher, runtime images

This package eliminates ad-hoc type duplication between Restate and system-bus. All sandboxed story execution must use these types to ensure contract stability. Story 3 added repo materialization and artifact export helpers for isolated sandbox runs. ADR-0221 phase 1 added the first explicit local-isolation primitives, and phase 2 now feeds those helpers into the real host-worker local sandbox path. ADR-0230 step 7 added the package-level Firecracker runner surface, and step 8 wires Restate DAG nodes to dispatch `handler: "microvm"` executions through that package.
...
- Firecracker microVM helpers now exist in `@joelclaw/agent-execution`, and Restate DAG nodes can now dispatch `handler: "microvm"` runs through that package via dynamic import; broader workload-dispatch defaults still remain on the proven local sandbox path until later ADR-0230 steps land
...
- The host-worker path now also has a deterministic non-LLM verification lane: `tool: "canary"` in `system/agent-dispatch` runs fixed subprocess scenarios through the same timeout/capture/writeback path, `bun scripts/verify-agent-dispatch-timeout.ts` forces the outer timeout path live without depending on an LLM to cooperate, `joelclaw status --agent-dispatch-canary` is the canonical on-demand health surface for that proof, the default `joelclaw status` envelope now shows the latest persisted canary truth, and the existing `check/system-health` signals schedule can optionally run the same proof when the live worker sets `HEALTH_AGENT_DISPATCH_CANARY_SCHEDULE=signals`
