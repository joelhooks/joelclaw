...
- Firecracker microVM helpers now exist in `@joelclaw/agent-execution`, but they are not the live dispatcher path yet: the package can build Firecracker REST request plans, manage microVM API sockets/processes, create snapshots, and use the initial workspace command-file bridge for single-use guest execution once step-8 wiring lands
