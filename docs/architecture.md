...
- The host-worker path now also has a deterministic non-LLM verification lane: `tool: "canary"` in `system/agent-dispatch` runs fixed subprocess scenarios through the same timeout/capture/writeback path, and `bun scripts/verify-agent-dispatch-timeout.ts` forces the outer timeout path live without depending on an LLM to cooperate
