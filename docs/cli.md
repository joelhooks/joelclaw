...
- New satellite Machines should use `scripts/setup-satellite-rig.sh` plus `docs/runbooks/satellite-rig-setup.md`. The bootstrap keeps satellites thin: repo + CLI + skills + raw session search + Central repair relay. Do not install Panda's k8s/Inngest/Redis/gateway stack on a satellite by default.
