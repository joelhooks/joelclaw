...
export const toolDefinitions = [
  ...
  { name: "herdr_dispatch_worker", description: "Send a task to a pi worker in one call: writes the task file, then REUSES the lane's existing pane (warm session) or opens one if the lane has none. The lane defaults to the taskId with trailing digit groups stripped, so recurring work (campaign-pulse-2026-07-25-1700) reuses one pane forever — pass an explicit lane to override. The worker returns its result via joelclaw notify send with data.taskId, arriving back in your queue as evidence. Pair with a fanout receipt; never block on the worker. Refuses past 4 open lanes: release a finished one first.", inputSchema: objectSchema({ taskId: string, label: string, task: string, lane: string }, ["taskId", "task"]) },
