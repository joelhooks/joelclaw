...
export const toolDefinitions = [
  ...
  { name: "herdr_dispatch_worker", description: "Spawn a pi worker pane for a task in one call: writes the task file, opens the pane beside the gateway loop, launches the worker. The worker returns its result via joelclaw notify send with data.taskId — it arrives back in your queue as evidence. Pair with a fanout receipt; never block on the worker.", inputSchema: objectSchema({ taskId: string, label: string, task: string }, ["taskId", "task"]) },
