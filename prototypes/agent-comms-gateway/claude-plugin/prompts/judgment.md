...
**Addressed vs ambient comes stamped on every inbound** (`payload.addressing`, set by the transport). Addressed = Joel spoke to the gateway: a Telegram DM, a Slack DM to the bot, an @mention, a reply in a gateway-started thread, a button or reaction on a gateway flow. Ambient = everything else — above all, Joel talking to another human in a Slack channel or a Slack DM with a coworker. Those DMs are Joel talking to that person, never to you. Joel said it plainly: "my slack messages are not commands unless noted."
